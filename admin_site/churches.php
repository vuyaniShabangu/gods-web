...
?>
    <div style="text-align: right; width: 100%;" >
        <button class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--raised mdl-button--colored" onclick="window.location='pages/add_church.php'";>
            Add New
        </button>
    </div><br/><br /><br/>

    <div id="churches_grid" style="width: 100%">
    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $.ajax({
                async: false,
                url: 'scripts/ChurchesScript.php?action=list',
                success: function (data) {
                    var result = $.parseJSON(data);

                    var table = constructTable(result);

                    $('#churches_grid').html(table);
                },
                error: function (data) {
                    alert("Error: " + data);
                }
        ...
        function constructTable(result) {
            ...
            html += "<th class=\"mdl-data-table__cell--non-numeric\">Church Name</th>";
            html += "<th class=\"mdl-data-table__cell--non-numeric\">Leading Member</th>";
            html +="<th class=\"mdl-data-table__cell--non-numeric\">Church Address</th>";
            html += "<th class=\"mdl-data-table__cell--non-numeric\">Description</th>";
            html += "</tr></thead><tbody>";
            var count = 0;
            ...
            for(var item in result)
            {
                ...
                html += "<td class=\"mdl-data-table__cell--non-numeric\">"  + result[count].leadingMember + "</td>";
                html += "<td class=\"mdl-data-table__cell--non-numeric\">" + result[count].address + "</td>";
                html += "<td class=\"mdl-data-table__cell--non-numeric\">" + result[count].description + "</td>";
                html += "</tr>";
        ...
        function editDelete(check, recordId) {
            ...
            dialog.querySelector('.edit').addEventListener('click', function () {
                window.location = 'pages/edit_church.php?q=' + recordId;
            });
            ...
            dialog.querySelector('.delete').addEventListener('click', function () {
                ...
                if (confirmed) {
                    $.ajax({
                        async: false,
                        url: 'scripts/ChurchesScript.php?action=delete&q=' + recordId,
                        success: function (data) {
                            if (data == "true") {
